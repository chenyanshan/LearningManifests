...
# 根据 rfc4106 标准，它需要 AES 密钥 和一个 4字节（32位）的盐值 (Salt) 组合在一起。
# 您在命令末尾指定了 128，这告诉内核 AES 密钥的长度是128位。
# 因此，总的密钥材料长度应该是：128位 (AES Key) + 32位 (Salt) = 160位。
# 160位等于 20字节，换算成十六进制字符串，需要 40个字符。
# KEY1="0x$(openssl rand -hex 20)"  # 0x6de61f32a637b96a1be22768076d46a9337326e8


cat <<EOF> clab.yaml
name: ipsec-aead-demo
topology:
  nodes:
    gw1:
        kind: linux
    ...
    host1:
        ...
        # 合并为单行命令:
        # 建立一条 IPsec 隧道状态，使用 ESP 协议，并采用 AES-GCM 这种集加密与认证于一体的高级算法 (aead) 来保护数据安全，其具体的密钥和长度由 $KEY 128 指定。
        # 根据 rfc4106 标准，它需要 AES 密钥 和一个 4字节（32位）的盐值 (Salt) 组合在一起。
        # 您在命令末尾指定了 128，这告诉内核 AES 密钥的长度是128位。
        # 因此，总的密钥材料长度应该是：128位 (AES Key) + 32位 (Salt) = 160位。
        # 160位等于 20字节，换算成十六进制字符串，需要 40个字符。
        # openssl rand -hex 20
        - ip xfrm state add src 10.0.1.2 dst 10.0.2.2 proto esp spi 0x9ba69051 reqid 0x9ba69051 mode tunnel aead 'rfc4106(gcm(aes))' 0x6de61f32a637b96a1be22768076d46a9337326e8 128
        ...
        - ip xfrm state add src 10.0.2.2 dst 10.0.1.2 proto esp spi 0xf2a13b9c reqid 0xf2a13b9c mode tunnel aead 'rfc4106(gcm(aes))' 0x6de61f32a637b96a1be22768076d46a9337326e8 128
